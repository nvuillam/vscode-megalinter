...
export const filterRemovedLintersFromSchema = <T extends RJSFSchema>(
  schema: T,
): T => {
...
export const extractGroups = (schema: RJSFSchema): SchemaGroups => {
  ...
  const descriptorWithLinters = new Set<string>(
    Object.values(linterDescriptorMap),
  );
  const descriptorList = descriptorEnums.filter((d) =>
    descriptorWithLinters.has(d),
  );
  ...
  linterList.forEach((id) =>
    ensureCategoryMeta(id, "linter", id, linterDescriptorMap[id]),
  );
  genericEnums.forEach((id) =>
    ensureCategoryMeta(id, "generic", resolveGenericLabel(id)),
  );
  ...
  Object.entries(properties).forEach(([propKey, propSchema]) => {
    ...
    const orderValue =
      typeof propSchema["x-order"] === "number"
        ? (propSchema["x-order"] as number)
    ...
    const assignToCategory = (
      ...
      ensureCategoryMeta(
        targetCategory,
        kind,
        categoryMeta[targetCategory]?.label || targetCategory,
        parentId,
      );
    ...
    const descriptorPrefix = descriptorPrefixes.find((p) =>
      propKey.startsWith(p),
    );
    if (descriptorPrefix) {
