...
export class ConfigTreeProvider implements vscode.TreeDataProvider<SectionNode> {
  ...
  getChildren(element?: SectionNode): vscode.ProviderResult<SectionNode[]> {
    ...
    if (!element) {
      const descriptorIds = this._sortCategoryIds(Object.keys(this._groups.descriptorKeys));
      ...
      const genericCategoryIds = this._sortCategoryIds(Object.keys(this._groups.genericCategoryKeys));
      const genericNodes = genericCategoryIds.map((categoryId) => {
        const hasValues = hasAnyKeySet(this._groups!.genericCategoryKeys[categoryId] || [], this._configKeys);
